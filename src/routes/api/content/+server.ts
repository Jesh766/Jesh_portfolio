...
import { normalizePortfolioContent } from '$lib/server/validation';
...
export const GET: RequestHandler = async ({ cookies, url }) => {
	...
	const content = preview ? (data?.draft_content ?? data?.content ?? {}) : (data?.content ?? {});
	return json({ content: normalizePortfolioContent(content) }, { headers: { 'cache-control': 'no-store' } });
};
